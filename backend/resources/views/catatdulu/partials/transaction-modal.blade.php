@php
    ...
    $defaultCategories = $txType === 'income'
        ? ['Gaji', 'Freelance', 'Investasi', 'Bonus']
        : ['Makanan', 'Transportasi', 'Belanja', 'Tagihan', 'Hiburan', 'Kesehatan'];
    $categoryOptions = ! empty($categories[$txType] ?? []) ? $categories[$txType] : $defaultCategories;
    $paymentMethods = ['BCA', 'Mandiri', 'GoPay', 'Cash', 'OVO'];
@endphp
<div id="tx-modal" class="modal hidden fixed inset-0 z-50 items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <form method="POST" action="{{ route('catatdulu.transactions.store') }}" class="bg-card rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        @csrf
        <input type="hidden" name="type" value="{{ $txType }}">
        <div class="px-6 py-4 border-b border-border flex items-center justify-between">
            <h3>Tambah {{ $txType === 'income' ? 'Pemasukan' : 'Pengeluaran' }}</h3>
        ...
        <div class="p-6 overflow-y-auto flex-1 space-y-4">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="amount" value="{{ old('amount') }}" step="0.01" placeholder="0" class="w-full h-10 px-3 rounded-lg border border-border bg-card text-sm pl-10" required>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="category_name" class="w-full h-10 px-3 rounded-lg border border-border bg-card text-sm">
                        @foreach ($categoryOptions as $categoryName)
                            <option value="{{ $categoryName }}" @selected(old('category_name') === $categoryName)>{{ $categoryName }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div>
                ...
                <input type="date" name="transaction_date" value="{{ old('transaction_date', date('Y-m-d')) }}" class="w-full h-10 px-3 rounded-lg border border-border bg-card text-sm" required>
            </div>
            <div>
                <label class="block text-xs font-semibold text-foreground mb-1.5">Catatan</label>
                <textarea name="notes" class="w-full min-h-[90px] p-3 rounded-lg border border-border bg-card text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" placeholder="Tambahkan catatan opsional...">{{ old('notes') }}</textarea>
            </div>
        </div>
        <div class="px-6 py-4 border-t border-border flex justify-end gap-2">
            ...
            <button type="submit" class="{{ $buttonClass }} bg-primary text-primary-foreground">Simpan Transaksi</button>
        </div>
    </form>
</div>
